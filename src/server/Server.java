...
public class Server {
	 ...
	 */
	private synchronized void broadcast(MessageCommand message) {
		
		// display message on console or GUI
		
		String messageLf = message.getConcreateMsg().getMessage() + "\n";
		String username = message.getUsername();
		
		sg.appendRoom(username + " : " + messageLf); // append in the room window
		...
		public void run() {
			...
			while (keepGoing) {
				...
				case ChatMessage.MESSAGE:
					MessageCommand mc = new MessageCommand(username,"msg",cm);
//					broadcast(username + ": " + message);
				...
				case ChatMessage.WHOISIN:
					...
					for (int i = 0; i < al.size(); ++i) {
						ClientThread ct = al.get(i);
						sb.append((i + 1) + ") " + ct.username + " since "
								+ ct.date + '\n');
					}
					MessageCommand mc2 = new MessageCommand(sb.toString(),"note");
					writeMsg(mc2);
					break;
				}
			}
